...
def status():
    return jsonify({
        ...
        "endpoints": ["/ui", "/playlists", "/albums", "/artists", "/devices", "/now_playing"]
    })
...
def airplay_full():
    """Return a list of AirPlay devices with their current volume and active status."""
    # Primary approach: iterate the AirPlay devices collection directly (most reliable).
    script_primary = '''
    ...
    # If the primary approach failed (dict error or returned explicit ERROR), try a name-based fallback.
    ...
    # Parse the tab-separated lines into JSON
    items = []
            ...
            if len(parts) >= 3:
                ...
                if name:
                    items.append({
                        "name": name,
                        "volume": vol_i,
                        "active": sel in ("true", "yes", "1")
                    })
    # Sort: active devices first, then by name (case-insensitive)
    try:
...
def play_music():
    ...
    if music_type == 'playlist':
        ...
        selection_block = (
            ...
            f'set theName to "Home Assistant Queue"\n'
