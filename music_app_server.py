...
def _watch_master_loop():
    ...
    while True:
        ...
        try:
            ...
            rp = get_repeat_enabled()
            if rp != last_repeat:
                last_repeat = rp
                _last_snapshot['repeat'] = rp
                _sse_publish('repeat', {"mode": rp})
        except Exception as e:
            app.logger.debug(f"watch master/shuffle/repeat error: {e}")
        time.sleep(itv)
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .devs{display:flex;flex-direction:column;gap:10px;padding-right:4px}
...
<body>
  ...
  <div class="grid cols">
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <div id="browseResults" class="devs"></div>
      <div id="pagination" class="row" style="justify-content:space-between;margin-top:24px;gap:8px">
        <div class="row" style="gap:8px">
          <button class="btn" onclick="changePage(currentPage - 1)" id="prevBtn">Prev</button>
          <span>Page <input type="number" id="pageInput" min="1" value="1" style="width:60px;text-align:center;border:1px solid var(--border2);border-radius:6px;background:#0f1520;color:var(--text);padding:4px"> of <span id="totalPages">1</span></span>
          <button class="btn" onclick="changePage(currentPage + 1)" id="nextBtn">Next</button>
        </div>
        <div class="row" style="gap:8px">
          <button class="btn" onclick="changePage(parseInt($('#pageInput').value) || 1)">Go</button>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="row" style="justify-content:space-between;margin-top:10px">
        <div class="row" style="gap:8px">
          <button class="btn" onclick="window.open('https://github.com/csilvertooth/music_app_server', '_blank')" title="View on GitHub">
...
async function loadNow(){
  try{
    ...
    updateRepeatButton(data.repeat || 'off');
...
async function toggleRepeat(){
  try{
    // Get current repeat mode first
    const currentMode = await call('/repeat','GET');
    let nextMode;
    if (currentMode.mode === 'off') {
      nextMode = 'all';
    } else if (currentMode.mode === 'all') {
      nextMode = 'one';
    } else if (currentMode.mode === 'one') {
      nextMode = 'off';
    } else {
      nextMode = 'all';
    }
    const r = await call('/repeat','POST',{mode: nextMode});
    if(r.ok){
      updateRepeatButton(r.mode);
...
function updateRepeatButton(mode){
  const btn = $('#btn_repeat');
  // Remove any existing indicator text
  const existingText = btn.querySelector('.repeat-indicator');
  if (existingText) existingText.remove();

  // Update styling and add text indicator
  if (mode === 'one') {
    btn.classList.add('btn-primary');
    btn.innerHTML += '<span class="repeat-indicator" style="position:absolute;top:2px;right:2px;font-size:10px;color:#fff;font-weight:bold;">1</span>';
    btn.setAttribute('title', 'Repeat One');
  } else if (mode === 'all') {
    btn.classList.add('btn-primary');
    btn.innerHTML += '<span class="repeat-indicator" style="position:absolute;top:2px;right:2px;font-size:10px;color:#fff;font-weight:bold;">A</span>';
...
function updateLetterNavigation(){
  ...
  allItems.forEach(item => {
    ...
    if(firstLetter.match(/[A-Z0-9]/)) letters.add(firstLetter);
  });
  const sortedLetters = Array.from(letters).sort();
  const allLetters = ['All', ...sortedLetters];
  const totalPages = Math.ceil(allLetters.length / lettersPerPage);
  const startIndex = (currentLetterPage - 1) * lettersPerPage;
  const endIndex = startIndex + lettersPerPage;
  const pageLetters = allLetters.slice(startIndex, endIndex);

  let html = pageLetters.map(letter => `<button class="btn" onclick="filterByLetter('${letter === 'All' ? '' : letter}')">${letter}</button>`).join('');

  // Add navigation buttons on a separate row if there are multiple pages
  if (totalPages > 1) {
    const prevDisabled = currentLetterPage === 1 ? 'disabled' : '';
    const nextDisabled = currentLetterPage === totalPages ? 'disabled' : '';
    html += `<div style="margin-top:8px;display:flex;gap:8px;justify-content:center"><button class="btn" onclick="changeLetterPage(${currentLetterPage - 1})" ${prevDisabled}>‹ Prev</button><button class="btn" onclick="changeLetterPage(${currentLetterPage + 1})" ${nextDisabled}>Next ›</button></div>`;
  }
...
def play_music():
    ...
    if music_type == 'song':
        ...
        if playlist and index:
            ...
            tell application "Music"
                try
                    {f'set current AirPlay devices to {{ {devices_script} }}' if devices else ''}
...
# ---- Media Player Endpoints for AirPlay Devices ----

def _un_slug(s: str) -> str:
    """Reverse of _safe_slug: convert slug back to original name."""
    return s.replace('_', ' ')

def get_media_players():
    """Return list of media player dicts for each AirPlay device."""
    try:
        master_now = _get_now_playing_dict()
        airplay_status = _read_airplay_full()
        airplay_volumes = _get_airplay_volumes()
    except Exception as e:
        app.logger.debug(f"get_media_players error: {e}")
        return []

    media_players = []
    for device in airplay_status:
        name = device['name']
        if not name:
            continue
        active = device.get('active', False)
        raw_vol = airplay_volumes.get(name, 0) or 0
        volume_level = max(0.0, min(1.0, raw_vol / 100.0))
        state = "playing" if (active and master_now.get('is_playing', False)) else "paused"
        shuffle_mode = "shuffle" if master_now.get('shuffle', False) else "off"
        repeat_mode = master_now.get('mode', 'off')
        device_slug = _safe_slug(name)
        entity_id = f"media_player.airplay_{device_slug}"
        media_players.append({
            "entity_id": entity_id,
            "friendly_name": f"AirPlay: {name}",
            "state": state,
            "volume_level": volume_level,
            "shuffle_mode": shuffle_mode,
            "repeat_mode": repeat_mode,
            "supported_features": ["play", "pause", "stop", "volume_set", "volume_step", "next_track", "previous_track", "repeat_set", "shuffle_set"],
            "device": name
        })
    return media_players

@app.route('/media_players', methods=['GET'])
def get_media_players_endpoint():
    """Return list of available media players (one per AirPlay device)."""
    try:
        _start_watchers_once()
    except Exception:
        pass
...
@app.route('/media_player/<entity_suffix>', methods=['GET'])
def get_media_player_status(entity_suffix):
    """Get status of a specific AirPlay media player."""
    if not entity_suffix.startswith('airplay_'):
        return jsonify({"error": "Not found"}), 404
    device_slug = entity_suffix[8:]  # remove 'airplay_'
    device_name = _un_slug(device_slug)
    players = get_media_players()
    for player in players:
        if player.get('device') == device_name:
            return jsonify(player)
    return jsonify({"error": "Not found"}), 404

@app.route('/media_player/<entity_suffix>/volume', methods=['POST'])
def set_media_player_volume(entity_suffix):
    """Set volume for a specific AirPlay media player."""
    if not entity_suffix.startswith('airplay_'):
        return jsonify({"error": "Not found"}), 404
    device_slug = entity_suffix[8:]
    device_name = _un_slug(device_slug)
    data = request.get_json(silent=True) or {}
    try:
        volume_level = float(data.get('volume_level', -1))
        if volume_level < 0 or volume_level > 1:
            raise ValueError
        level = int(round(volume_level * 100))
        ok = _set_airplay_device_volume(device_name, level)
        if not ok:
...
@app.route('/media_player/<entity_suffix>/play', methods=['POST'])
def play_media_player(entity_suffix):
    """Start playback using this AirPlay device as the output."""
    if not entity_suffix.startswith('airplay_'):
        return jsonify({"error": "Not found"}), 404
    device_slug = entity_suffix[8:]
    device_name = _un_slug(device_slug)
    # Set this device as the output and start playing
    payload = {"devices": device_name}
    result = set_devices_endpoint_internal([device_name])
    if not result.get('status', False):
        return jsonify({"error": "Failed to select device"}), 500
    # Now play
    res = run_applescript('tell application "Music" to play')
    if isinstance(res, dict):
        return jsonify({"error": res.get('error', 'Play failed')}), 500
    return jsonify({"success": True})

@app.route('/media_player/<entity_suffix>/pause', methods=['POST'])
def pause_media_player(entity_suffix):
    """Pause playback."""
    if not entity_suffix.startswith('airplay_'):
        return jsonify({"error": "Not found"}), 404
    script = 'tell application "Music" to pause'
    result = run_applescript(script)
    if isinstance(result, dict):
        return jsonify({"error": result.get('error', 'Pause failed')}), 500
    return jsonify({"success": True})

@app.route('/media_player/<entity_suffix>/stop', methods=['POST'])
def stop_media_player(entity_suffix):
    """Stop playback."""
    if not entity_suffix.startswith('airplay_'):
        return jsonify({"error": "Not found"}), 404
    script = 'tell application "Music" to stop'
    result = run_applescript(script)
    if isinstance(result, dict):
        return jsonify({"error": result.get('error', 'Stop failed')}), 500
    return jsonify({"success": True})

@app.route('/media_player/<entity_suffix>/next', methods=['POST'])
def next_media_player(entity_suffix):
    """Next track."""
    if not entity_suffix.startswith('airplay_'):
        return jsonify({"error": "Not found"}), 404
    script = 'tell application "Music" to next track'
    result = run_applescript(script)
    if isinstance(result, dict):
        return jsonify({"error": result.get('error', 'Next failed')}), 500
    return jsonify({"success": True})

@app.route('/media_player/<entity_suffix>/previous', methods=['POST'])
def previous_media_player(entity_suffix):
    """Previous track."""
    if not entity_suffix.startswith('airplay_'):
        return jsonify({"error": "Not found"}), 404
    script = 'tell application "Music" to previous track'
    result = run_applescript(script)
    if isinstance(result, dict):
        return jsonify({"error": result.get('error', 'Previous failed')}), 500
    return jsonify({"success": True})

@app.route('/media_player/<entity_suffix>/repeat', methods=['POST'])
def set_media_player_repeat(entity_suffix):
    """Set repeat mode for media player (global setting)."""
    data = request.get_json(silent=True) or {}
    # Home Assistant sends repeat mode as string: "off", "one", "all"
    repeat_mode = data.get('mode', 'off')
    if repeat_mode not in ("off", "one", "all"):
        repeat_mode = "off"
    ok = set_repeat_enabled(repeat_mode)
    _sse_publish('repeat', {"mode": get_repeat_enabled()})
    if not ok:
        return jsonify({"error": "Failed to set repeat"}), 500
    return jsonify({"success": True})

@app.route('/media_player/<entity_suffix>/shuffle', methods=['POST'])
def set_media_player_shuffle(entity_suffix):
    """Set shuffle mode for media player (global setting)."""
    data = request.get_json(silent=True) or {}
    # Home Assistant sends boolean for shuffle
    shuffle_enabled = bool(data.get('shuffle', False))
    ok = set_shuffle_enabled(shuffle_enabled)
    _sse_publish('shuffle', {"enabled": bool(get_shuffle_enabled())})
    if not ok:
        return jsonify({"error": "Failed to set shuffle"}), 500
    return jsonify({"success": True})

def set_devices_endpoint_internal(devices_list):
    """Internal version to set devices by list."""
    if not devices_list:
        # No-op if empty; don't clear devices implicitly
        return {"status": True, "applied": []}
    # Build AppleScript list of names safely
    name_list = ", ".join([f'"{applescript_escape(n)}"' for n in devices_list])
    script = f'''
    tell application "Music"
        try
            set tlist to {{{name_list}}}
    ...
    result = run_applescript(script)
    if isinstance(result, dict):
        return {"status": False, "error": result.get('error', 'AppleScript error')}
    if isinstance(result, str) and result.startswith("ERROR:"):
        return {"status": False, "error": result}
    applied = []
    if isinstance(result, str) and result:
        applied = [s.strip() for s in result.split(',') if s and s.strip()]
    # Push an immediate AirPlay snapshot so UIs refresh without waiting for poll
    try:
        statuses = _read_airplay_full()
        volumes = _get_airplay_volumes()
        for item in statuses:
        ...
        pass
    return {"status": True, "applied": applied}
