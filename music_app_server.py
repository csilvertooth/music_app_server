    ...
    while True:
        ...
        try:
            ...
            if changed or meta_changed or restarted:
                ...
                # Prefetch and cache current and likely-next artwork in the background
                try:
                    # Current album — ensure cached
                    if album:
                        app.logger.debug(f"prefetch: current album='{album}'")
                        threading.Thread(target=lambda: _album_art_bytes(album), daemon=True).start()
                    # Best effort: probe next track's album to prefetch
                    script_next = '''
                    tell application "Music"
                        try
                            set nt to next track
                            set nm to (name of nt as text)
                            set ar to (artist of nt as text)
                            set al to (album of nt as text)
                            return nm & tab & ar & tab & al
                        on error
                            return "\t\t"
                        end try
                    end tell
                    '''
                    nxt = run_applescript(script_next)
                    if isinstance(nxt, str) and nxt:
                        parts = nxt.split("\t")
                        next_alb = parts[2] if len(parts) > 2 else ""
                        next_art = parts[1] if len(parts) > 1 else ""
                        if next_alb:
                            # Only resolve via _album_art_bytes once to avoid recursion-in-thread
                            def _pref():
                                try:
                                    app.logger.debug(f"prefetch: next album='{next_alb}' (artist='{next_art}')")
                                    _album_art_bytes(next_alb)
                                except Exception:
                                    pass
                            threading.Thread(target=_pref, daemon=True).start()
                except Exception:
                    pass
            else:
    ...
    <div class="card">
      <div class="now">
        ...
        <div>
          ...
          <div class="transport">
            <button class="btn btn-circle" title="Previous" onclick="action(this, ()=>call('/previous','POST'))" aria-label="Previous" id="btn_prev"></button>
            <button id="pp" class="btn btn-circle btn-primary" title="Play/Pause" aria-label="Play/Pause" onclick="action(this, playPause)"></button>
            <button class="btn btn-circle" title="Next" onclick="action(this, ()=>call('/next','POST'))" aria-label="Next" id="btn_next"></button>
          </div>
        </div>
      </div>
      <div class="footer">
...
def artwork():
    ...
    if album and not force_refresh:
        ...
        if data:
            app.logger.debug(f"/artwork: serve from cache for album='{album}' artist='{artist}'")
            return Response(data, mimetype=(mime_cached or _guess_image_mime(data)))
    ...
    if album:
        _write_album_cache(album, artist, data)
        app.logger.debug(f"/artwork: cached album='{album}' artist='{artist}' -> {ARTWORK_DIR}")
...
def _try_read_album_cache(album: str, artist: str | None) -> tuple[bytes | None, str | None]:
    ...
    for ext in ("jpg", "png", "jpeg"):
        p = _album_cache_path(album, artist, ext)
        try:
            with open(p, "rb") as f:
                data = f.read()
            ...
            continue
    app.logger.debug(f"artwork cache: MISS for album='{album}' artist='{artist}' in {ARTWORK_DIR}")
    return None, None
...
def _write_album_cache(album: str, artist: str | None, data: bytes) -> None:
    ...
    mime = _guess_image_mime(data)
    ext = "jpg"
    if isinstance(mime, str) and "png" in mime:
        ext = "png"
    path = _album_cache_path(album, artist, ext)
    try:
        with open(path, "wb") as f:
            f.write(data)
        app.logger.debug(f"artwork cache: WROTE {path} ({len(data)} bytes)")
    except Exception as e:
