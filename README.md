...
[![GitHub Repository](https://img.shields.io/badge/GitHub-Repository-blue.svg)](https://github.com/csilvertooth/apple_music_server_macos)
...
- `POST /media_player/airplay_<device_slug>/shuffle` body: `{shuffle:true|false}` → Set shuffle mode (global)
- `POST /media_player/airplay_<device_slug>/repeat` body: `{repeat:"off"|"one"|"all"}` → Set repeat mode (global)
...
- `shuffle_mode`: "off" or "shuffle"
- `repeat_mode`: "off", "all", or "one"
